...
unsigned char *img; // global image
...
//created global variables so that each thread can access them without having to create instances for each thread
...
int main(int argc, char *argv[]){
   ...
   /* Will contain the raw image */
    ...
    long threadID;
    pthread_t* thread_handles = malloc(sizeof(pthread_t) * threads); // creates thread handles

   struct timeval begin, end;
    /*** start timing here ****/
    gettimeofday(&begin, 0);

    for (threadID = 0; threadID < threads; threadID++) // loop to create threads
    {
        pthread_create(&thread_handles[threadID], NULL, genImage, (void*) threadID); 
        // creates threads and runs the genImage function
    }

    for(threadID = 0; threadID < threads; threadID++)
    {
        pthread_join(thread_handles[threadID], NULL);
        // waits for each thread to be done and will end them 
    }
    free(thread_handles);
...
void *genImage(void* thread) // function called by pthread_create
{
   ray r;
    long threadID = (long) thread;

    int xStart = (WIDTH / threads) * threadID; // start of chunk thread is processing
    int xEnd = (WIDTH / threads) * (threadID + 1); // end of chunk thread is processing
